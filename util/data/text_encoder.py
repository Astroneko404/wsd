...
from util.data import tokenizer
from util import constant

import tensorflow as tf
from datetime import datetime
# Reserved tokens for things like padding and EOS symbols.
SYMBOL_PAD = constant.PAD
SYMBOL_UNK = constant.UNK
SYMBOL_START = constant.BOS
SYMBOL_END = constant.EOS
SYMBOL_NONTARGET = constant.NONTAR
RESERVED_TOKENS = [SYMBOL_PAD, SYMBOL_UNK, SYMBOL_START, SYMBOL_END, SYMBOL_NONTARGET]
...
for token in RESERVED_TOKENS:
  RESERVED_TOKENS_DICT[token] = token_idx
  token_idx += 1
...
class SubwordTextEncoder(TextEncoder):
  ...
  def _tokens_to_subtoken_ids(self, tokens):
    ...
    ret = []
    for token in tokens:
      if token in RESERVED_TOKENS_DICT:
          ret.append(RESERVED_TOKENS_DICT[token])
      else:
          ret.extend(
