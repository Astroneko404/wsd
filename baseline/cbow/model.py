...
class CBOW(nn.Module):
    ...
    def __init__(self, vocab_size, sense_size, emb_dimension):
        """Initialize model parameters.
        ...
        self.sense_embeddings = nn.Embedding(vocab_size, emb_dimension, sparse=True)
    ...
    def forward(self, pos_context_words, pos_sense, neg_context_words):
        """Forward process.
        ...
        emb_u = self.u_embeddings(pos_u)
        emb_v = self.v_embeddings(pos_v)
        score = torch.dot(emb_u, emb_v).squeeze()
        score = torch.sum(score, dim=1)
        score = F.logsigmoid(score)
        neg_emb_v = self.v_embeddings(neg_v)
        neg_score = torch.bmm(neg_emb_v, emb_u.unsqueeze(2)).squeeze()
