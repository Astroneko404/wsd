...
if __name__ == '__main__':
    ...
    dataset_processed_path = data_path + "/mimic_test/processed"
    abbr_inventory_path = data_path + "/abbr_inventory.pkl"
    example_note_path = "/data/batch4/500K_By_Sources_2013_output/PEGREMO_21/doc.9071.txt"
    ...
    wsd = AbbrDisambiguation2(
        train_processed_path=dataset_paths.mimic_new_train_folder,
    ...
    cursor = conn.cursor()
    cursor.execute("""
    with t as (select cluster_id, "textLiteral",abbr_id, ROW_NUMBER() over (partition by abbr_id,cluster_id order by cluster_instance.id) as row
from cluster_instance
where cluster_id != -1
  and sense_id is not null
  )
select cluster_id, "textLiteral",abbr_id
from t
where row = 1;
    """)
    cluster_data = [(row[0],row[1],row[2]) for row in cursor]
    ...
    for (cluster_id,text,abbr_id) in cluster_data:
        raw,wsd_result = wsd.process_single_text(text)
        result[(abbr_id,cluster_id)] = wsd_result[0]["wsd"]



    import pickle

    with open("./result_mimic_annotated.pkl", "wb") as result_file:
